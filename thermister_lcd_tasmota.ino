...
const int thermistor_output1 = 35; // brown
const int thermistor_output2 = 32; // white
...
const char index_html[] PROGMEM = R"rawliteral(
  ...
  <option value="tuya" %TUYA%>Tuya
    ...
    <label for="idh"><span>Heating Device Id</span><input  id="idh" type="text" maxlength="22"  name="tuya_deviceh" value="%TUYA_DEVICEH%"></label>
     <label for="fidc"><span>Cooling Device Id</span><input  id="idc" type="text" maxlength="22"  name="tuya_devicec" value="%TUYA_DEVICEC%"></label>
     <label for="fiph"><span>Heating Device IP</span><input  id="fiph" type="text" maxlength="22"  name="iph" value="%IPH%"></label>
     <label for="fipc"><span>Cooling Device IP</span><input  id="fipc" type="text" maxlength="22"  name="ipc" value="%IPC%"></label>
...
void handleSetting() {
  ...
  if (server.hasArg("tuya_keyh") ) {
    String tuya_device_arg = server.arg("tuya_keyh");
    if (tuya_device_arg.length() <= 18 && strcmp(tuya_keyh, tuya_device_arg.c_str()) != 0) {
      strcpy(tuya_keyh, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  ...
  if (server.hasArg("tuya_deviceh") ) {
    String tuya_device_arg = server.arg("tuya_deviceh");
    if (tuya_device_arg.length() == 22 &&  strcmp(tuya_deviceh, tuya_device_arg.c_str()) != 0) {

      strcpy(tuya_deviceh, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  ...
  if (server.hasArg("tuya_keyc") ) {
    String tuya_device_arg = server.arg("tuya_keyc");
    if (tuya_device_arg.length() == 18 &&  strcmp(tuya_keyh, tuya_device_arg.c_str()) != 0 ) {
      strcpy(tuya_keyh, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
  ...
  if (server.hasArg("tuya_devicec") ) {
   ...
   String tuya_device_arg = server.arg("tuya_devicec");
    if (tuya_device_arg.length() == 22 &&  strcmp(tuya_devicec, tuya_device_arg.c_str()) != 0) {
      strcpy(tuya_devicec, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  ...
  if (server.hasArg("ipc") ) {
    String tuya_device_arg = server.arg("ipc");
    if (tuya_device_arg.length() < 16 &&  strcmp(ipc, tuya_device_arg.c_str()) != 0) {
      strcpy(ipc, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  }
  ...
  if (server.hasArg("iph") ) {
    String tuya_device_arg = server.arg("iph");
    if (tuya_device_arg.length() < 17 &&  strcmp(iph, tuya_device_arg.c_str()) != 0) {
      strcpy(iph, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  }
  ...
  if (server.hasArg("tuya_api_key") ) {
    String tuya_device_arg = server.arg("tuya_api_key");

    if (tuya_device_arg.length() < 33 &&  strcmp(tuya_api_key, tuya_device_arg.c_str()) != 0) {
     
      strcpy(tuya_api_key, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  }
  if (server.hasArg("tuya_api_client") ) {
    String tuya_device_arg = server.arg("tuya_api_client");
    if (tuya_device_arg.length() < 21 &&  strcmp(tuya_api_client, tuya_device_arg.c_str()) != 0) {
 
      strcpy(tuya_api_client, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  }
  ...
  if (server.hasArg("tuya_host") ) {
    ...
    String tuya_device_arg = server.arg("tuya_host");
    if (tuya_device_arg.length() < 100 &&  strcmp(tuya_host, tuya_device_arg.c_str()) != 0)  {
      strcpy(tuya_host, tuya_device_arg.c_str());
      changed = true;
      reboot = true;
    }
  }
...
void handleCalib() {
  ...
  Serial.println("has anything changed");
  if (changed) {
...
bool readSettings(char *buffer) {
  if (SPIFFS.exists("/settings.txt")) {
    ...
    Serial.println("settings do exist");
   ...
   Serial.println("settings dont exist");
  return false;
}
